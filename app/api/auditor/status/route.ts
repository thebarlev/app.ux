...
export async function GET(req: Request) {
  try {
    ...
    const { data: pageStates } = await admin
      .from("auditor_scan_pages")
      .select("state")
      .eq("scan_id", parsed.data.scanId)

    const pageRows = Array.isArray(pageStates) ? pageStates : []
    const pagesTotal = pageRows.length
    const pagesSuccess = pageRows.filter((row: any) => row?.state === "fetched" || row?.state === "extracted").length
    const pagesFailed = pageRows.filter((row: any) => row?.state === "failed").length
  ...
  // (e.g. rules stage wrote rules but scan update failed, or timing gap). Build issues from rules.
    ...
    const rawScoreTotal =
      typeof scan.score_total === "number" && Number.isFinite(scan.score_total)
        ? scan.score_total
        : null
    const scoreReady = scan.status === "done" && rawScoreTotal !== null
    ...
    return NextResponse.json(
      {
        ...
        screenshot_url,
        score_total: scoreReady ? rawScoreTotal : null,
        score_search: scoreReady ? safeReportPublic.score_search : null,
        score_ai: scoreReady ? safeReportPublic.score_ai : null,

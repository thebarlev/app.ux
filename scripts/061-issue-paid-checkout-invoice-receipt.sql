-- ====================================================
-- 061 - Service-only issuance: paid checkout -> VOW invoice_receipt
-- ====================================================
-- Purpose:
-- - Replace initial purchase issuance from `receipt` to `invoice_receipt`
-- - Keeps idempotency via billing_documents.unique(checkout_session_id)
...
begin
  ...
  select c.company_name, coalesce(c.registration_number, c.tax_id), c.email, c.mobile_phone
    into v_buyer_company_name, v_buyer_tax_id, v_buyer_email, v_buyer_phone

-- ============================================================================
-- 137 · Field 1265 carries a NET unit price, per the uniform-file spec
-- ============================================================================
--
...
--
-- "הוראות להפקת קבצים במבנה אחיד", version 1.31, page 13:
--
--   1265  מחיר ליחידה ללא מע"מ      X9(12)v99   241-255   ח/ר
--   1267  סך סכום לשורה             X9(12)v99   271-285   ח/ר
--         הבהרה: הכמות בשורה * מחיר ליח' ללא מע"מ בניכוי הנחת השורה
--
-- So 1267 = 1264 x 1265 - 1266, and because 1265 is by its own name net, 1267 is net too.
--
-- ── WHAT WAS MEASURED IN THE BUILT FILE ─────────────────────────────────────
--
-- Document 1004, D110, before this change:
--
--   1264 quantity  0001000        1265 unit  +00000000011800
--   1266 discount  0              1267 line  +00000000011800
--
-- The multiplication holds, but on the wrong input: 118.00 includes VAT, in a field
-- defined as excluding it. The document header says so itself — 1221 ("סכום המסמך לאחר
-- הנחות ללא מע"מ") is +00000000010000 for the same document. So the line and the header
-- disagreed by exactly the VAT.
--
-- v_subtotal is that net figure, already computed at line 171 of this body as
-- round(v_total / (1 + v_vat_rate/100), 2). It is used rather than a fresh calculation so
-- the line and the header cannot drift.
--
-- ⚠️ In the English flow v_vat_rate is 0 and line 167 sets v_subtotal := v_total, so this
-- change is a no-op there. v_subtotal is used unconditionally rather than behind an `if`,
-- because a branch would be a second place for the two to disagree.
--
-- ── ⛔ THE PAYMENT LINE IS DELIBERATELY LEFT GROSS ──────────────────────────
--
-- line_number 90, payment_metadata kind='payment', keeps v_total. It maps to D120, which
-- carries the amount the customer actually paid — VAT included. Document 1004 emitted
-- +00000000011800 there and that is correct. Normalising it to net would turn a valid
-- D120 into an invalid one.
--
-- Item line net, payment line gross. Two fields, two purposes. The generator that wrote
-- this change asserts the payment line still holds v_total before the file is produced.
--
-- ── PROVENANCE ──────────────────────────────────────────────────────────────
--
-- The body below is `pg_get_functiondef` from the live database, read over POSTGRES_URL,
-- with exactly two lines changed — verified by diff:
--
--   -    v_total,        +    v_subtotal,     (unit_price)
--   -    v_total,        +    v_subtotal,     (line_total)
--
-- The repo's own copy of 136 was lost at zero bytes; see scripts/136-RECOVERED.sql.
-- Production, not the repository, is the base for this file.
--
-- ⛔ Existing documents are NOT changed. This is the write path. 1000-1007 keep the
-- values they were issued with and re-render exactly as delivered.
--
-- No install guard: both overloads were confirmed present in the same read that produced
-- this body, and a migration re-deriving a fact it just measured is a second thing that
...
-- Run separately after the commit. Expect the item line net and the payment line gross:
--
--   select li.line_number, li.quantity, li.unit_price, li.line_total,
--          li.payment_metadata->>'kind' as kind
--     from public.document_line_items li
--     join public.documents d on d.id = li.document_id
--    where d.document_number = '<the next auditor invoice>'
--    order by li.line_number;
--
-- For a 118.00 charge: line 1 -> unit 100.00, total 100.00, kind auditor_subscription
--                      line 90 -> total 118.00, kind payment

select '137-auditor-line-unit-price-net.sql applied' as migration;

-- ============================================================================
-- 136-RECOVERED · the production definition, recovered from the live database
-- ============================================================================
--
-- ⛔ THIS FILE IS DOCUMENTATION. IT IS NOT MEANT TO BE RUN.
--
-- The original scripts/136-auditor-invoice-name-and-receipt-half.sql was applied to
-- production successfully — its effects are visible in the built uniform file: documents
-- 1004-1007 carry a D120 and their line description reads "מנוי אודיטור" rather than
-- "מנוי /auditor". But the FILE was lost. Both commits that touched it stored it at
-- ZERO bytes (53d51c9 and c28e38b), and on disk it survived as a single tab character.
-- Its ROLLBACK survived intact at 18,834 bytes; the forward file did not.
--
-- How it happened, plainly: the file was written, then opened in TextEdit for review, and
-- committed without anyone measuring it afterwards. `git add` accepted zero bytes in
-- silence and `git status` stayed clean, because the emptiness IS what was committed. The
-- check that would have caught it — `wc -c` after the commit — was not run.
--
-- So the repository was lying: it claimed a migration existed that it did not hold. This
-- file stops that. The text below is `pg_get_functiondef` from the live database, read
-- directly over POSTGRES_URL, byte for byte and unedited. 20,945 characters.
--
-- ── ⚠️ TWO OVERLOADS, AND ONLY ONE OF THEM IS CORRECT ABOUT OWNERSHIP ───────
--
-- Production holds two:
--
--   (uuid, uuid, boolean)  20,945 chars  — this one. It stamps documents.company_id with
--                          p_issuer_company_id, which is the issuing dealer. Correct.
--   (uuid, uuid)              493 chars  — a thin SQL wrapper delegating to
--                          ..._service_impl, which stamps v_charge.company_id — the
--                          BUYER. That is the ownership defect, it is on the frozen list,

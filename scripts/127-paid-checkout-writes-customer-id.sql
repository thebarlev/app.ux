-- ====================================================
-- 127 - Paid checkout writes customer_id  (wiring · path 5 of 7)
-- ====================================================
-- Attaches the buyer to a customers row and records it on the document, for the
-- paid-checkout issuance path.
--
-- ── IT BUILDS ON scripts/125, AND IT DOES NOT NEED IT ───────────────────────
-- The body here is scripts/125's body (which is scripts/073 plus customer_email)
-- plus the three edits below. Intended order is 126 → 125 → this file, so the
-- payment path takes one reviewed change at a time. But this file is
-- self-sufficient: it contains 125's customer_email change as well, so if 125 is
-- skipped, applying this one still delivers both.
--
-- Against the LIVE definition (verified as scripts/073 from pg_get_functiondef),
-- the total change is 31 lines: 4 functional + comments.
--
-- ── THE THREE EDITS ─────────────────────────────────────────────────────────
--   1. declare v_customer_id uuid;
--   2. call public.resolve_customer(...) after the buyer row is loaded
--   3. append customer_id to the dynamic column list, after customer_email
--
-- Everything else is byte-identical to 125, verified by diff.
--
-- ── WHY THE CALL SITS WHERE IT DOES ─────────────────────────────────────────
-- Immediately AFTER `if v_buyer_company_name is null then v_buyer_company_name :=
-- 'לקוח'; end if;`. That is deliberate and it is the only correct position:
-- 'לקוח' is one of the three literals public.is_placeholder_customer_name refuses
-- to match on, so a buyer whose company row has no name gets a customer row of its
-- own rather than being merged with every other nameless buyer. Resolving BEFORE
-- the fallback would pass a NULL name, which lands on the same rule by a different
-- route; resolving after makes the intent legible.
--
-- The buyer's tax id is coalesce(registration_number, tax_id) and CAN be null, in
-- which case match key 1 is unavailable and the email decides. The email is
-- available here only because scripts/125 added it.
--
-- ── IT FAILS OPEN, AND THAT IS THE OPPOSITE OF THE FORM PATH ────────────────
-- The resolver call is wrapped in `exception when others then v_customer_id :=
-- null`. The buyer has already been charged by the time this function runs, so a
-- paid checkout with no document is strictly worse than a document with a null
-- customer_id — the second is a row that can be repaired, the first is a support
-- ticket and a refund.
--
-- issueDocumentAction makes the opposite choice for the same reason inverted:
-- nothing is charged, no number is reserved, and the user can press the button
-- again, so it fails closed and shows an error rather than issuing a document that
-- cannot go into the regulatory file.
--
-- ⚠️ THE CONSEQUENCE, STATED SO IT IS NOT A SURPRISE AT STEP 4:
-- Failing open means this path CAN still produce a document with a null
-- customer_id. The step-4 check on public.documents must therefore either exempt
-- this path or the failure must be treated as blocking — and that decision has to
-- be made before step 4 is written, not discovered by it. Not decided here.
--
-- Unlike the TypeScript paths, the swallowed exception writes no failure row:
-- billing_failures is written by application code, and this function has no access
-- to it. A resolver failure here is therefore SILENT. That is the weakest point in
-- this file. It is acceptable only because the resolver's realistic failure modes
-- (bad company_id, RLS) cannot occur on a service-role definer path — and it is
-- exactly the kind of silence that hid the ambiguity bug for 81 days.
--
-- ⚠️ LIVE PAYMENT PATH. Called from app/api/billing/cardcom/confirm/route.ts:303,
-- .../indicator/route.ts:444 and .../issuance/repair-paid-checkouts/route.ts:87.
--
-- ── PREREQUISITE ────────────────────────────────────────────────────────────
-- scripts/124 must be applied — it is, as of 2026-08-10 — or resolve_customer does
-- not exist and every call raises undefined_function. Note that the exception
-- handler above would SWALLOW exactly that, leaving customer_id null with no
-- signal. Confirm the function exists before relying on this file's effect:
--   select oid::regprocedure from pg_proc where proname = 'resolve_customer';
-- ====================================================
...
begin
  ...
  -- Customer register. Resolved AFTER the 'לקוח' fallback above, deliberately:
  -- that literal is one of the three placeholder names resolve_customer refuses
  -- to match on, so a buyer whose company row has no name gets a customer of its
  -- own instead of being glued to every other nameless buyer.
  --
  -- FAILS OPEN. The buyer has already paid by the time this function runs; a paid
  -- checkout with no document is worse than a document with a null customer_id,
  -- which is a row that can be repaired later. The exception detail is discarded
  -- rather than re-raised, and v_customer_id simply stays null.
  begin
    v_customer_id := public.resolve_customer(
      p_issuer_company_id, v_buyer_company_name, v_buyer_tax_id, v_buyer_email
    );
  exception when others then
    v_customer_id := null;
  end;
  ...
  if v_doc_id is null then
    ...
    if exists (select 1 from information_schema.columns where table_schema='public' and table_name='documents' and column_name='customer_id') then
      ...
      v_vals := array_append(v_vals, quote_nullable(v_customer_id));
    end if;

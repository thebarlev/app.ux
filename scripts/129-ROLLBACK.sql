...
-- ROLLBACK for 129
-- ====================================================
-- Restores public.issue_auditor_charge_invoice_receipt_service(uuid, uuid, boolean)
-- to the definition scripts/085 installed: identical in every respect except that it
-- does not write public.auditor_invoice_documents.
--
-- The body below is scripts/085 lines 13-385 verbatim, not 129 with the block
-- removed, so this cannot produce a third variant of a billing function.
--
-- WHAT RUNNING THIS RE-ENABLES
-- Ownership stays correct — that half lives in the TypeScript change, not here — but
-- it becomes unverifiable. New auditor documents stop being cross-referenced, and
-- the query that answers "was this stamped on the right dealer" goes blind again.
--
-- Rows already written by 129 are kept. Reverting the mechanism does not erase its
-- history.
--
-- ⚠️ Do not run this while the renewals route is scheduled. Correct-but-unobservable
-- is the state we deliberately refused to ship.
-- ====================================================

-- ====================================================
-- 100 - Fix: INSERT has more target columns than expressions
-- ====================================================
-- Replaces dynamic array-based INSERT with explicit INSERT...SELECT
-- to avoid column/expression count mismatch in document_line_items.
-- ====================================================
...
begin
  ...
  v_payment_meta := jsonb_build_object(
  ...
  -- Line item: use INSERT...SELECT to avoid "INSERT has more target columns than expressions"
  if v_has_item_date and v_has_currency_li and v_has_payment_metadata then
    ...
    select v_doc_id, v_charge.company_id, 1, 'מנוי /auditor – ' || v_plan_name, current_date, v_total, 1, v_total, coalesce(v_charge.currency, 'ILS'), v_payment_meta;
  elsif v_has_item_date and v_has_currency_li then
    insert into public.document_line_items (
      document_id, company_id, line_number, description, item_date, unit_price, quantity, line_total, currency
    )
    select v_doc_id, v_charge.company_id, 1, 'מנוי /auditor – ' || v_plan_name, current_date, v_total, 1, v_total, coalesce(v_charge.currency, 'ILS');
  elsif v_has_payment_metadata then
    insert into public.document_line_items (
      document_id, company_id, line_number, description, unit_price, quantity, line_total, payment_metadata
    )
    select v_doc_id, v_charge.company_id, 1, 'מנוי /auditor – ' || v_plan_name, v_total, 1, v_total, v_payment_meta;
  else
    insert into public.document_line_items (
      document_id, company_id, line_number, description, unit_price, quantity, line_total
    )
    select v_doc_id, v_charge.company_id, 1, 'מנוי /auditor – ' || v_plan_name, v_total, 1, v_total;
  end if;

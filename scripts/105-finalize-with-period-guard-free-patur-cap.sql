...
-- ====================================================
-- Purpose:
-- - Enforce a hard cap for both 'free' and 'free_patur' plans
-- - Keep paid plan behavior unchanged (no cap; overages handled elsewhere)
-- Notes:
-- - Enforcement uses subscriptions.plan_snapshot_documents_limit (snapshot)
-- ====================================================
...
begin
  ...
  v_is_free_like := v_sub.plan_id in ('free','free_patur');

  if v_is_free_like then
  ...
  if v_is_free_like and v_used >= v_limit then

...
-- THE DEFECT
--
-- Every call failed, without exception:
--
--   42702  column reference "document_id" is ambiguous
--          It could refer to either a PL/pgSQL variable or a table column.
--
-- `returns table (ok boolean, document_id uuid, document_number text)` declares three
-- OUT parameters, which are PL/pgSQL variables. Three places in the body then use the
-- bare identifier `document_id` where a column of that name also exists, and PL/pgSQL
-- substitutes identifiers across the whole statement — including an INSERT column list
-- and an ON CONFLICT target. Postgres cannot tell which was meant, so it refuses.
--
...
--
-- `#variable_conflict use_column` tells PL/pgSQL that a column wins whenever a name is
-- ambiguous. Renaming the OUT parameters would also work, but it would change the JSON
-- keys the function returns and break every caller that reads `.document_id`.
--
-- The pragma applies to the WHOLE body, so every occurrence of the three OUT parameter
-- names was counted in the production definition before writing this:
--
--   ok               0 occurrences in the body. Supplied positionally at all five
--                    return sites. Nothing to resolve.
--   document_id      4 — L343 an INSERT column list, L413 an INSERT column list,
--                    L415 an ON CONFLICT target, L404 inside a `--` comment.
--                    Those three code sites ARE the ambiguity. None is ever READ as
--                    a variable.
--   document_number  2 — both inside string literals ('document_number', passed to

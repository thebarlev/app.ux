...
-- ROLLBACK for 125
-- ====================================================
-- Restores public.issue_paid_checkout_document_service to the definition
-- scripts/073 installed: identical in every respect except that it does not write
-- documents.customer_email.
--
-- The body below is scripts/073 lines 16-357 verbatim. It is not a hand-written
-- reconstruction and it is not 125 with the block deleted — it is the original file,
-- so applying it cannot introduce a third variant of this function.
--
-- WHAT RUNNING THIS RE-ENABLES
-- Paid-checkout documents stop recording the buyer's email. Documents issued while
-- 125 was live KEEP theirs; this changes what is written from now on, not what was
-- written. If the customer register is already resolving buyers, match key 2 (email)
-- goes dark for this path and those buyers fall through to key 3 (exact name) or
-- key 4 (a new row every time) — so the register starts accumulating duplicates for
-- the paid-checkout path specifically.
--
-- ⚠️ This is the live paid-checkout path. A customer has paid by the time it runs.
-- ====================================================

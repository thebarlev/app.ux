...
-- 131 ROLLBACK
-- ====================================================
-- Open this in a second tab BEFORE running 131, not after.
--
-- Restores public.issue_auditor_charge_invoice_receipt_service(uuid,uuid,boolean) to
-- its post-129 definition: customer_name and customer_tax_id written, customer_id
-- not. The body below is scripts/129's, copied by the same script that produced 131,
-- so the two cannot drift.
--
-- ⚠️ WHAT THIS DOES NOT UNDO
-- Any document already issued while 131 was installed keeps its customer_id, and it
-- should — that value is correct, and rolling back the function is not a reason to
-- blank a field on an issued tax document. Nor does this remove customer rows
-- resolve_customer created; they are a register, not a side effect.
--
-- After running this, the uniform-file measure starts growing again on every new
-- auditor invoice. That is the state 131 exists to end, so treat a rollback here as
-- temporary.
-- ====================================================
...
select '⛔ 131 ROLLED BACK' as result;

-- ============================================================================
-- 136-ROLLBACK · restore the function exactly as 133 left it
-- ============================================================================
--
-- Reinstalls 133's body byte for byte: the Hebrew product name goes back to
-- 'מנוי /auditor – ' and the payment line item stops being written.
--
-- ⛔ WHAT REVERTING COSTS
--
-- New invoices go back to reading `מנוי auditor/ – בסיסי` on a tax document, and to
-- having no receipt half at all on a document titled "חשבונית מס קבלה". Both are the
-- defects 136 fixes, so this restores a known-wrong state rather than a safe one.
--
-- It does NOT undo anything on documents 136 already produced — their line items stay as
-- written. Run this only if 136 itself misbehaves.
-- ============================================================================
...
begin
  if to_regprocedure('public.issue_auditor_charge_invoice_receipt_service(uuid,uuid,boolean)') is null then
    raise exception '136-ROLLBACK: the three-argument overload is not installed.';
  end if;
end
$guard$;
...
do $verify$
begin
  if position('''kind'', ''payment''' in pg_get_functiondef(to_regprocedure('public.issue_auditor_charge_invoice_receipt_service(uuid,uuid,boolean)'))) <> 0 then
    raise exception '136-ROLLBACK FAILED: the payment line item is still present. Nothing committed.';
  end if;
  raise notice '136-ROLLBACK done: the receipt half is gone and the product name reads /auditor again.';
end
$verify$;

commit;

select '136-ROLLBACK.sql reverted — no receipt half, and the product name is a URL path again' as migration;

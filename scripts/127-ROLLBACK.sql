...
-- ROLLBACK for 127
-- ====================================================
-- Restores public.issue_paid_checkout_document_service to scripts/125's version:
-- customer_email is still written, customer_id is not, and resolve_customer is not
-- called.
--
-- The body below is scripts/125's body verbatim, not 127 with the additions
-- stripped, so this cannot produce a third variant.
--
-- WHAT RUNNING THIS RE-ENABLES
-- Paid-checkout documents stop carrying customer_id, so they are invisible to
-- field 1225 of the BKMV file. Customers already created by this path stay, and
-- documents already pointing at them keep pointing at them.
--
-- ⚠️ If step 4's check constraint on public.documents is already in place and it
-- requires customer_id for invoice_receipt, this rollback makes every paid
-- checkout FAIL at the constraint instead of merely issuing without a customer.
-- Drop the check before running this.
--
-- To go further back — no customer_email either — use scripts/125-ROLLBACK.sql,
-- which restores scripts/073.
-- ====================================================

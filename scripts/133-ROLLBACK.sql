...
--
-- The pre-133 body throws 42702 on every call. Reverting does not restore a working
-- state; it restores a function that cannot issue a tax document, for any charge, ever.
-- Two charges are already sitting with money taken and no document because of it.
--
-- So run this only to undo an unrelated problem introduced by the deploy around 133 —
...
select '133-ROLLBACK.sql reverted — the function throws 42702 again' as migration;

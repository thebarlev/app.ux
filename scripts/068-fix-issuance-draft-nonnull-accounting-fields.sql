-- ====================================================
-- 068 - Fix: issuance draft must not set NOT NULL accounting fields to NULL
-- ====================================================
-- Evidence:
-- - confirm/route.ts documentIssuance failed with:
--   code 23502: null value in column "paid_amount" violates not-null constraint
--
-- Root cause:
-- - documents.paid_amount / credited_amount / outstanding_balance are NOT NULL (default 0).
-- - In 067 we inserted issued documents as 'draft' but set those columns to NULL explicitly.
--
-- Fix:
-- - When inserting draft, set:
--   paid_amount = v_amount (or v_ev.total_amount)
--   credited_amount = 0
...
begin
  ...
  -- Prefer full name for regulatory docs; keep company name if both exist.
...
begin
  ...
  v_customer_display := coalesce(nullif(v_contact_full_name, ''), v_company_name);
  if v_contact_full_name is not null and v_contact_full_name <> '' and v_company_name <> '' and v_company_name <> v_contact_full_name then
    v_customer_display := v_contact_full_name || ' (' || v_company_name || ')';
  end if;

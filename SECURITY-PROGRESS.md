...
במפת הזיכויים: החלטה על סימן הסכום, יצירת תבנית `credit_note` (בלעדיה `pdf-service.ts:979` זורקת),
קביעת מספר ההתחלה **לפני** ההנפקה הראשונה, יישוב הוולידציה בטופס, החלטה על סמנטיקת הקישור,
והכרעה מול מסלול הקבלה השלילית הקיים. BKMV אינו צעד ברשימה — הוא אינו מייצא שום סוג, כי
`lib/regulatory/bkmv/spec.ts` אינו מאוכלס וה-builder מסרב.

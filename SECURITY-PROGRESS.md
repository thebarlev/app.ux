...
## חסימת חשבונית זיכוי — הושלמה ואומתה ב-preview · מוזגה ל-main

**ענף:** `security/credit-note-block` @ `f6ebcc6`, נחתך מ-`main@ff9516c`. קומיט מיזוג `b1ca234`.
**השיטה:** אותה שיטה שאושרה בחסימת ה-auditor — `const CREDIT_NOTE_BLOCKED: boolean = true`,
מקודד ולא משתנה סביבה, כהוראה הראשונה שמתבצעת. ה-annotation load-bearing.

### מה הניע את זה — לחיצה כושלת אחת שרפה מספר מסמך
`generate_document_number` נקראת ב-`lib/document-helpers.ts:298` והמספר נשמר ב-`:321-328`,
כ-760 שורות **לפני** בניית ה-PDF ב-`:1057`. אלה קריאות PostgREST נפרדות, כל אחת מתחייבת בנפרד,
ולכן אין טרנזקציה עוטפת שכישלון מאוחר יכול לבטל. בהיעדר תבנית `credit_note`,
`lib/pdf-service.ts:979` זורקת `TEMPLATE_NOT_FOUND`, שמגיעה כ-`{ ok: false }` ב-`:254`
ומוחזרת ב-`document-helpers.ts:1078` — **אחרי** שהמספר התחייב. אין בקוד שום הפחתה של
`current_number` ואין מחיקה של הטיוטה הממוספרת, ו-`generate_document_number` יוצרת את הרצף
עם `is_locked = true`. כלומר: כל טיוטה חדשה שנכשלת צרכה מספר לצמיתות ברצף נעול — מכפתור שהיה חי
בפרודקשן.

### ⛔ תיקון לנימוק שלמעלה — הראיה כנראה מופרכת · נבדק 9.8.2026 ⛔
**הנימוק הישן נשאר למעלה במלואו ואינו נמחק. הוא כנראה לא נכון, וזה מה שנמצא:**

`lib/pdf-service.ts:387-399` מגדיר `TEMPLATE_TYPE_ALIASES`, ובו **`credit_note: "tax_invoice"`**
(וגם `self_credit_note: "tax_invoice"`). כלומר בחירת התבנית לזיכוי אינה מחפשת תבנית `credit_note`
אלא נופלת לאותה משפחה של `invoice_receipt`, `quote`, `proforma` ועוד. ובפרודקשן **המשפחה הזאת
מתיישבת לתבנית קיימת** — 106 חשבוניות מס/קבלה ו-19 חשבוניות מס הונפקו בהצלחה דרכה.

**המשמעות:** `pdf-service.ts:979` כנראה **לא** הייתה זורקת `TEMPLATE_NOT_FOUND` על זיכוי, ולכן
"לחיצה כושלת שורפת מספר" לא היה קורה מהסיבה שנרשמה. איני יכול לאמת זאת בוודאות בלי להנפיק זיכוי,
וזה חסום — ולכן "כנראה" ולא "בהכרח".

**מה שלא משתנה: החסימה עומדת, על הבסיס השני.** לחיצה אחת יוצרת רצף עם `is_locked = true`
ו-`starting_number` שרואה החשבון לא הכריע עליו, ו-`initializeSequence` מסרבת לשנות אותו אחר כך
(`sequence_already_locked`). **זה שינוי מסד בלתי הפיך**, והוא ההצדקה שנשארת תקפה — היא השכבה
השנייה בטבלה שמתחת. ההצדקה תקפה; הראיה שנרשמה לה כנראה לא.

### שלוש שכבות, לפי סדר החשיבות
| שכבה | מה | קובץ |
|---|---|---|
| **עצירת ההקצאה** | `issueCreditNoteAction` מחזירה כישלון לפני `issueDocumentAction`, ולכן מספר לא נמשך בכלל. מחזירה ולא זורקת, במבנה ש-`CreditNoteFormClient.tsx:665` כבר מטפל בו | `credit-note/actions.ts` |
| **עצירת יצירת הרצף** | `lockStartingNumberAction` מסרבת **לסוגי זיכוי בלבד**. זה היה המעקף השני: דיאלוג "בחירת מספר מסמך ראשון" מגיע ל-`initializeSequence`, שמכניס שורה עם `is_locked = true` (`lib/document-helpers.ts:111-121`), ואחרי מיגרציה 118 הנעילה נאכפת — לחיצה אחת הייתה קובעת לצמיתות את מספר ההתחלה לפני שרואה החשבון הכריע | `dashboard/documents/actions.ts` |
| **הדלתות** | שלושה שומרי מסלול + הסרת האריח | 4 קבצים |

### ⛔ הטעות שנתפסה בבדיקה — תיקייה סטטית מצילה מסלול דינמי ⛔
בסבב הראשון הושם `notFound()` בענף `creditNote` שב-`[documentType]/page.tsx`. הוא עבר `tsc`
ו-build — **והטופס נפתח במלואו ב-preview.** הסיבה: קיימת תיקייה סטטית `new/creditNote/`
כאחות של `[documentType]/`, ומקטע סטטי גובר על דינמי, ולכן הכתובת מעולם לא הגיעה לקובץ שחסמנו.
**כל ארבעת הסוגים מוצלים כך**, ולכן `[documentType]/page.tsx` הוא קוד מת. השומר שם נשאר בכוונה
(עלות אפס, ותקף אם ההצללה תוסר). מתועד ב-`FOLLOWUPS.md` עם הכלל: לבדוק ב-`ls` אם קיימת תיקייה
סטטית לפני עריכת הקובץ, ולאמת בדפדפן ולא רק ב-build.

### מה אומת ב-preview — חמש בדיקות, כולן עברו
שני מסלולי הטופס והמסלול המצל מחזירים **404**, כולל `?draftId=` · האריח נעלם ונשארו שלושה ·
**טופס חשבונית מס-קבלה נטען על 1156 בלי דיאלוג מספור** — זו הבדיקה שהצמדת התנאי לסוג נועדה להגן עליה.

### מה הושאר בכוונה
`saveCreditNoteDraftAction` — טיוטה אינה נושאת מספר ואינה צורכת דבר, ובלי מסלול טופס אין ממשק
שמגיע אליה. `issueNegativeReceiptForInvoiceReceiptAction` ותת-העץ שלו — הוא עשוי להיות הפתרון,
לא הבעיה. `assertBkmvSpecComplete`. כל סוג מסמך אחר — התנהגות זהה בדיוק.

### מה זה לא
זו **חסימה, לא תיקון.** הפער שמאפשר הנפקת זיכוי תקינה נשאר פתוח במלואו, ומסודר לפי סדר ביצוע
במפת הזיכויים שמתחת.

### מפת הזיכויים — עודכנה מול המפרט 1.31, 9.8.2026
שלוש שאלות שהיו פתוחות ברשימה **נסגרו מתוך המפרט**, ולא בהכרעה שלנו:

**קוד המסמך — `330`.** נספח 1, מילה במילה: `330  חשבונית מס זיכוי`. וגם `710  זיכוי רכש` לרכש.
נספח 1 הוא טבלה סגורה (סעיף 2.4.ה: "יש להשתמש רק בערכים המופיעים בטבלה זו בלבד").

**סימן הסכום — חיובי.** זו הייתה השאלה הראשונה ברשימה, והמפרט עונה עליה במפורש. הבהרה 1:
> משמעות המסמך בהנהלת חשבונות הינה עפ"י סוג המסמך ולא עפ"י ערכו. לדוגמא, חשבונית חיובית תגדיל
> הכנסה. תעודת זיכוי חיובית תקטין הכנסה שכן משמעות המסמך "תעודת זיכוי" הינה הקטנת הכנסה.

וסעיף 2.4.י"ב:
> ברשומות מסוג "כותרת מסמך" או "פרטי מסמך" (רשומות מסוג C100, D110), ערכים של שדות יוצגו תמיד
> בערך חיובי (+), למעט שדות שמשמעותם הקטנה של סכום המסמך / סכום השורה, שיוצגו בערך שלילי (-).

והערת נספח 1: `כאשר ערך המסמך שלילי, ההשפעה הפוכה`.
**כלומר סכומי זיכוי נרשמים חיוביים; הזיכוי מבוטא בקוד 330 ולא בסימן.** שלילי נשמר להנחה בלבד.

**הקישור למסמך המקורי — שדות `1256` ו-`1257` ב-`D110`**, שניהם `ח/ר` (רשות). הבהרה 6:
> כאשר המסמך הנידון מבוסס על מסמך אחר יש לציין את סוג המסמך עליו מתבססים ואת מספרו. לדוגמא,
> כאשר חשבונית מבוססת על תעודת משלוח (כשתעודת המשלוח "נסגרה" לחשבונית) אזי מסמך הבסיס של
> החשבונית תהיה תעודת משלוח זו. כך גם כאשר תעודת משלוח מבוססת על הזמנה או קבלה המבוססת על חשבונית.

הדוגמאות אינן נוקבות בזיכוי, אך "מסמך הנידון מבוסס על מסמך אחר" חל עליו ישירות. שני השדות ריקים
בייצוא היום — הם ברשימת 23 השדות ללא מקור ב-`docs/regulatory/bkmv/mapping.md`, כי אין
`base_document_id` במסד.

**מה שנשאר פתוח ברשימה:** קביעת מספר ההתחלה **לפני** ההנפקה הראשונה · יישוב הוולידציה בטופס ·
סמנטיקת הקישור בצד המוצר (הטופס אינו דורש הפניה למקור: `sourceDocumentId` נקרא מפרמטר URL
ואינו ב-payload, והקישור נוצר אחרי ההנפקה דרך `createDocumentLinkAction`) · והכרעה מול מסלול
הקבלה השלילית הקיים · ותבנית: לפי התיקון שלמעלה, `credit_note` נופל למשפחת `tax_invoice`, ולכן
ייתכן שאין צורך בתבנית נפרדת — **לאמת לפני שמסתמכים על כך.**

**BKMV אינו צעד ברשימה יותר.** נכון ל-9.8.2026 המייצא כן מאוכלס ומייצא 305/320/400/300; `330`
נעדר במכוון מטבלת הקודים ב-`lib/regulatory/bkmv/codes.ts`, כדי שזיכוי לא יהיה ניתן לייצוא בשקט
כל עוד הנפקתו חסומה.
